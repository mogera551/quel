...
const getBindTextFromTemplate:BindTextFn = (node:Node) => getTemplateByUUID(node.textContent?.slice(3) ?? "")?.dataset[BIND_DATASET] ?? "";
 ...
 */
export function getBindTextByNodeType(
  node: Node, 
  nodeType: NodeType
): string {
